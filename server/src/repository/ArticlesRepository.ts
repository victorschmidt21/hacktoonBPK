...
ORDER BY a.created_at DESC;`
  ...
  public async getByUser(
    ...
    try {
      const [result] = await db.query(
        ...
        FROM tb_artigo a 
        LEFT JOIN tb_user cu ON cu.idUser = a.creator_id
        LEFT JOIN tb_colaboradores c ON c.article_id = a.article_id 
        LEFT JOIN tb_user u ON u.idUser = c.user_id 
        ...
        for (const colaborator of articleData.colaborators) {
          await connection.query(
            `
                        ...
                        VALUES (?, ?);`,
            [insertedArticleId, colaborator.idUser]
          );
        }
      }
  ...
  public async update(
    ...
    try {
      ...
      if (articleData.status !== undefined) {
        ...
        updateValues.push(articleData.status.toLowerCase());
      }
      ...
      if (updateFields.length > 0) {
        await connection.query(
          `UPDATE tb_artigo SET ${updateFields.join(
            ", "
          )} WHERE article_id = ?`,
          updateValues
        );
      ...
      if (articleData.colaborators) {
        await connection.query(
          `DELETE FROM tb_colaboradores WHERE article_id = ?;`,
          [articleId]
        );

        if (
          Array.isArray(articleData.colaborators) &&
  ...
  public async delete(
    ...
    try {
      ...
      await connection.query(
        `DELETE FROM tb_comentarios WHERE article_id = ?;`,
        [articleId]
      );
      await connection.query(`DELETE FROM tb_revisao WHERE article_id = ?;`, [
        articleId,
      ]);
      await connection.query(`DELETE FROM tb_avaliacao WHERE article_id = ?;`, [
        articleId,
      ]);
      await connection.query(
        `DELETE FROM tb_area_tematica WHERE article_id = ?;`,
        [articleId]
      );
      await connection.query(
        `DELETE FROM tb_palavra_chave WHERE article_id = ?;`,
        [articleId]
      );
      await connection.query(
        `DELETE FROM tb_colaboradores WHERE article_id = ?;`,
      ...
      await connection.query(`DELETE FROM tb_artigo WHERE article_id = ?;`, [
        articleId,
      ]);
