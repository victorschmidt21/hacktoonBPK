...
export default class ArticlesRepository {
    ...
    public async getAll(req: Request, res: Response): Promise<void> {
        try {
            let result = await db.query("SELECT a.article_id, a.created_at, a.title AS tittle, JSON_OBJECT('idUser', creator.idUser, 'userName', creator.userName, 'url_img_user', creator.url_img_user, 'userEmail', creator.userEmail, 'type', creator.type) AS user, a.resumo, a.url_arquivo AS url, a.status, a.tematic_area, a.version, a.key_words, COALESCE(JSON_ARRAYAGG(JSON_OBJECT('idUser', u.idUser, 'userName', u.userName, 'url_img_user', u.url_img_user, 'userEmail', u.userEmail, 'type', u.type)), JSON_ARRAY()) AS colaborators FROM tb_artigo a LEFT JOIN tb_user creator ON creator.idUser = a.creator_id LEFT JOIN tb_colaboradores c ON c.article_id = a.article_id LEFT JOIN tb_user u ON u.idUser = c.user_id GROUP BY a.article_id ORDER BY a.created_at DESC;");
